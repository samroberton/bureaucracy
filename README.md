...
statecharts][Harel], and using them to manage the run-time state of an
application's user interface.

`bureaucracy` provides the hierarchy of faceless machines that make the (app's)
world go round.

It is heavily inspired by Kevin Lynagh's talks about building UIs with Harel
statecharts.  You can think of a lot of what this library tries to facilitate as
a slightly different take on Day 8 / Mike Thompson's excellent [re-frame]. Many
thanks to both Kevin and Mike.
